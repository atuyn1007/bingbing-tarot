...
const TarotCard = ({ card, isRevealed, size = 'normal', showOrientation = true, variant = 'minimal', rotateReversed = true }) => {
  ...
  return (
    <div className={`${sizeClasses} relative tarot-card-frame tarot-card-frame-${size} tarot-card-surface ${shouldShowArtwork ? 'tarot-card-surface-artwork' : ''}`}>
      ...
      {isRevealed && (
        <div className={`absolute inset-0 overflow-hidden rounded-[1.6rem] ${shouldShowArtwork ? '' : 'border border-stone-900/10 bg-[linear-gradient(160deg,_rgba(255,252,246,1)_0%,_rgba(246,237,224,1)_100%)] p-3 shadow-[0_24px_48px_rgba(93,66,33,0.16)]'}`}>
          {!shouldShowArtwork ? <div className="absolute inset-3 rounded-[1.2rem] border border-stone-800/10" /> : null}
          <div
            className={`relative flex h-full flex-col items-center justify-center ${shouldShowArtwork ? '' : 'rounded-[1.15rem] bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.95),_rgba(245,237,226,0.86)_52%,_rgba(229,213,191,0.92)_100%)] px-3 text-center'}`}
