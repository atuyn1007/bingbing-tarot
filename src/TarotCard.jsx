...
const TarotCard = ({ card, isRevealed, size = 'normal', showOrientation = true, rotateReversed = true }) => {
  ...
  return (
    <div className={`${sizeClasses} relative tarot-card-frame tarot-card-frame-${size} tarot-card-surface ${shouldShowArtwork ? 'tarot-card-surface-artwork' : ''}`}>
      ...
      >
        <div className="tarot-card-face tarot-card-back-face rounded-[1.6rem] border border-black/10 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.92),_rgba(224,214,198,0.72)_45%,_rgba(111,86,53,0.88)_100%)] shadow-[0_22px_45px_rgba(60,42,23,0.18)]">
          ...
          <div className="absolute inset-0 flex items-center justify-center" aria-hidden="true">
            <div className="flex h-16 w-16 items-center justify-center rounded-full border border-white/40 bg-white/10 text-white">
              ☉
            </div>
          </div>
        </div>

        <div className={`tarot-card-face tarot-card-front-face overflow-hidden rounded-[1.6rem] ${shouldShowArtwork ? '' : 'border border-stone-900/10 bg-[linear-gradient(160deg,_rgba(255,252,246,1)_0%,_rgba(246,237,224,1)_100%)] p-3 shadow-[0_24px_48px_rgba(93,66,33,0.16)]'}`}>
          {!shouldShowArtwork ? <div className="absolute inset-3 rounded-[1.2rem] border border-stone-800/10" /> : null}
          <div className={`relative flex h-full flex-col items-center justify-center ${shouldShowArtwork ? '' : 'rounded-[1.15rem] bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.95),_rgba(245,237,226,0.86)_52%,_rgba(229,213,191,0.92)_100%)] px-3 text-center'}`}>
            {shouldShowArtwork ? (
              <div className="tarot-card-artwork-wrap">
                <img
                  src={artworkSrc}
                  alt=""
                  className={`tarot-card-artwork ${rotateReversed && card.isReversed ? 'is-reversed' : ''}`}
