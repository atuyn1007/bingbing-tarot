create or replace function public.claim_daily_tarot(p_today_card jsonb)
returns table (
  id uuid,
  nickname text,
...
declare
  ...
  today_key text := ((now() at time zone 'Asia/Shanghai')::date)::text;
...
revoke all on function public.claim_daily_tarot(jsonb) from public;
revoke all on function public.claim_daily_tarot(jsonb) from anon;
grant execute on function public.claim_daily_tarot(jsonb) to authenticated;
